...
export default function HistoryTab({nickname, isMyProfile}: HistoryProps) {
  ...
  const getListByType = async () => {
    if (selectType === '전체 활동 내역') {
      ...
      const Result = await getActivityPlogInfo(
        nickname,
        activityPageNumber,
        10,
      );
      ...
      const Result = await getActivityTreeInfo(
        nickname,
        activityPageNumber,
        10,
      );
  ...
  const changeType = (type: string) => {
    if (type !== selectType) {
      ...
      setPageNumber(
        activityPageNumber => activityPageNumber - activityPageNumber,
      );
    }
  };
    ...
    <>
      {isImageModalOpen && (
        <ImageDetailModal
          ...
          closeFnt={() => setImageModalOpen(false)}
          imageURL={imageURL}
        />
      )}
      ...
      <View
        style={isMyProfile ? styles.historySelect : styles.historySelectYours}>
        <SingleSelect
          dataList={dataList}
      ...
      <View style={styles.historyBox}>
        <View style={styles.historyInner}>
          {activityList.content.length === 0 ? (
            <AppText
              children="아직 활동 기록이 없습니다."
            ...
            <FlatList
              ...
              renderItem={({item}) => {
                ...
                if (activityType === 'PLOG') {
                  return (
                    <TouchableOpacity
                      onPress={() => openModal(fileUrl)}
                      key={activityId}
                      style={styles.historyBoxSection}>
                      <View style={styles.historyBoxLeft}>
                        <View style={styles.historyBoxDate}>
                          <AppText
                            style={styles.historyBoxDateText}
                            children={`${createTime
                              .slice(0, 10)
                              .replaceAll('-', '.')}`}
                          />
                        ...
                        <View style={styles.historyBoxDetail}>
                          ...
                          <View style={styles.historyBoxDetailText}>
                            ...
                            <AppText style={styles.historyBoxContent}>
                              {plogging?.ploggingLength && (plogging.ploggingLength/1000).toFixed(2)}km
                            </AppText>
                          </View>
                        </View>
                          ...
                          <View style={styles.historyBoxDetailText}>
                            ...
                            <AppText style={styles.historyBoxContent}>
                              {plogging?.ploggingTime
                                ? `${Math.floor(
                                    plogging.ploggingTime / 3600,
                                  )}시간 ${Math.floor(
                                    (plogging.ploggingTime % 3600) / 60,
                                  )}분 ${plogging.ploggingTime % 60}초`
                                : '0시간 0분 0초'}
                            </AppText>
                          </View>
                        </View>
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      <View style={styles.historyBoxLeft}>
                        ...
                        <FastImage
                          source={require('@/assets/profile_image/certificate_img.png')}
                          style={styles.historyBoxAnimal}
                          resizeMode={FastImage.resizeMode.contain}
                        />
                      </View>
                      <View style={styles.historyBoxRight}>
                        <FastImage
                          source={require('@/assets/img_icon/medal_icon.png')}
                          style={styles.historyBoxMedal}
                        />
                        <AppText style={styles.historyBoxTitle}>
                          나의 증서 보기
                        </AppText>
